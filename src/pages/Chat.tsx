...
export default function Chat({ isWsConnected, stompClient, messagesByChannel, isAdmin, userDepartment, allDepartments }: ChatProps) {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if (message.trim() === '' || !isWsConnected || !stompClient) return;
  ...
  const handleUnsendMessage = (messageId: string | number) => {
    if (!messageId || !isWsConnected || !stompClient) {
      console.error("Cannot unsend message without ID or connection.");
      return;
    }

    // Publish a delete request to the backend. The UI will update when the server broadcasts a confirmation.
    stompClient.publish({
        ...
        <main className="flex-1 flex flex-col">
          ...
          <div className="p-6 border-t border-white/10 bg-black/20">
            <form onSubmit={handleSendMessage} className="flex items-center gap-4">
              ...
              />
              <button type="submit" className="w-14 h-14 bg-cyan-600 hover:bg-cyan-500 rounded-xl flex items-center justify-center text-white transition-all shadow-lg shadow-cyan-600/20 disabled:bg-slate-700 disabled:shadow-none" disabled={!isWsConnected}>
                <Send size={24} />
              </button>
            </form>
          </div>
